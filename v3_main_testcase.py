...
logFile = './logs/testcaseV3_log.txt'
testcaseFile = 'testcaseV3.xlsx'
testcaseResultFile = 'testcaseV3_result.xlsx'
...
if __name__ == '__main__':
    for idx in range(0,len(testcase)):
        ...
        gaTimeCriteria = testcase.loc[idx,"Kriteria Waktu GA"]

        # ACO Parameters
        acoTimeCriteria = testcase.loc[idx,"Kriteria Waktu ACO"]
        ...
        for testIdx in range(1, 4):
            ...
            acoDistance, acoProgress, acoNGeneration, acoTimeExecution = antColonyOptimization(city=city, nAnts=nAnts, rho=rho, alpha=alpha, beta=beta, initialPheromne=initialPheromne, startTime=acoStartTime, timeCriteria=acoTimeCriteria, log=log, DEBUG=DEBUG)
            ...
            gaAcoStartTime = time.time()
            # Start Genetic Algorithm & Ant Colony Optimization Algorithm process
            gaAcoDistance, gaAcoProgress, gaAcoNGeneration, gaAcoTimeExecution = gaAcoAlgorithm(city=city, nAnts=nAnts, rho=rho, alpha=alpha, beta=beta, initialPheromne=initialPheromne, routes=newPop, startTime=gaAcoStartTime, timeCriteria=acoTimeCriteria, log=log, DEBUG=DEBUG)
            ...
            plt.legend()
            plt.ylabel('Distance')
            plt.xlabel('Generation, GA: '+str(gaNGeneration)+' (1-'+ str(gaNGeneration) +'), ACO: '+str(gaAcoNGeneration)+' ('+str(gaNGeneration+1)+'-'+str(gaNGeneration+gaAcoNGeneration)+')')
            ...
            testcase.loc[idx, "Runtime GA Percobaan " + str(testIdx)] = round(gaTimeExecution/60,4)
            testcase.loc[idx, "Runtime ACO Percobaan " + str(testIdx)] = round(acoTimeExecution/60,4)
            testcase.loc[idx, "Runtime GA-ACO Percobaan " + str(testIdx)] = round((gaTimeExecution+gaAcoTimeExecution)/60,4)
