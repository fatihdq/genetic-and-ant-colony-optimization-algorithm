...
def geneticAlgorithm(population, popSize, genCriteria, log, DEBUG=False):
    tabulistResult = []
    finalPopulation = []
    pop = []
    ...
    while looping:
        ...
        if len(sameResult) == 0:
            sameResult.append(Route(pop[0]).routeDistance())
        else:
            if sameResult[-1] == Route(pop[0]).routeDistance():
                sameResult.append(Route(pop[0]).routeDistance())
            else:
                sameResult = []
        ...
        finalPopulation = pop
        if (DEBUG) or (looping==False) or (i == 0): 
            log.printToLog("\n====================================== GENERASI KE " + str(
                i + 2) + "======================================>")
    ...
    nGeneration = i
    log.printToLog("Total GENERATION = "+ str(nGeneration))
    ...
    return finalPopulation, bestDistance, progress, nGeneration